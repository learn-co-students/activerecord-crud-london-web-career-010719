...
def can_be_created_in_a_block(params={})
  Movie.create do |m|
    params.each{ |k, v| m.send("#{k}=", v) }
    m.title ||= "Home Alone"
    m.release_date ||= 1990
  end
end
...
def can_destroy_a_single_item
  ...
  Movie.destroy(Movie.find_or_create_by(title: "That One Where the Guy Kicks Another Guy Once").id)
end
